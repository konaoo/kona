...
- 币种显示优先取后端返回的 `curr`（仅当 `curr` 无效时才按市场类型回退），用于避免 B股被 `market='a'` 误显示为人民币。

### 4.3 `v1.0.22` 分析页日历修复点

- 日历接口返回 `INVALID_CALENDAR_PERIOD` 时，前端会根据返回的 `selectable` 自动纠正年月并重试一次。
- 历史周期命中缓存后仍会后台回源刷新，避免跨月后 `selectable` 过期导致“无法切回当月”。
