...
(async () => {
  ...
  // 4. 直接导航到贵州茅台 (sh600519) 的详情页
  const detailUrl = 'http://localhost:5173/app/asset/sh600519';
  console.log(`Navigating directly to detail page: ${detailUrl}`);
  ...
  // 截图 1：点击前
  await page.screenshot({ path: path.join(__dirname, 'modal_before_click.png') });
  console.log('Screenshot saved: modal_before_click.png');
  ...
  // 截图 2：点击后
  await page.screenshot({ path: path.join(__dirname, 'modal_after_click.png') });
  console.log('Screenshot saved: modal_after_click.png');
