...
4. 单只持仓“当日盈亏”与汇总“今日收益”分口径处理：  
   - 单只显示可保留休市冻结值；  
   - 汇总仅计入当前开市市场（US 扩展时段活跃也计入）。  
5. 目标是“现价展示 + 当日盈亏口径”在 Web / App 同步一致。  
...
4. A/Fund 节假日覆盖（防交易日历覆盖不足导致误判）：
   - 文件：`/Users/kona/Desktop/kaka/kona_repo/kona_tool/market_holidays.json`
   - 已内置 2026 年上交所公告休市日（元旦/春节/清明/劳动节/端午/中秋/国庆）
   - `force_closed` 优先级高于交易日历与工作日回退，用于保证混合开市日口径正确（如 2026-02-23）
...
1. 单只显示：休市市场继续显示最近有效价格与冻结日涨跌。
2. 汇总口径：`investDayPnl/investDayPnlRate` 仅计入开市市场。
3. US 若处于盘前/盘后扩展时段活跃，允许计入汇总。
4. 全市场休市时，汇总“今日收益”为 `0`。
5. 若无可用行情证据，单只与汇总才兜底到成本价链路。
6. 首页投资资产改为价格口径重算（非固定成本）。
7. 混合开市日（例如 `2026-02-23`：A 休市、HK 开市）：
   - A 单只可显示冻结值；
   - 汇总“今日收益”不得计入 A 的冻结值，仅统计 HK/US（含 US 扩展时段）等活跃市场。
8. 若后续年份交易所日历覆盖不足，先补 `market_holidays.json`，再发布；禁止仅依赖“工作日=开市”的回退口径。
