...
- `v1.0.22`：管理后台改版（全后台统一卡片/按钮/表格组件风格）；菜单精简为“数据概览/用户管理/邀请码管理/运营配置/接口管理”；删除审计菜单、前端审计路由与后端审计查询导出接口；用户管理页改为“全部=总资产>0”口径并支持四列表头升降序；用户详情重构为“现金/其他/负债”摘要 + 持仓累计盈亏与类型展示（用户维度 24 小时缓存）。
