...
4. 协作执行特例（仅 Web 端）：默认“完成即直推 `main`”，推送后由你验收；Flutter/Backend 仍按常规确认流程执行。
...
- `v1.0.5`：Web 分析页对齐 App；排行去分类 + 查看更多；港股/美股金额按汇率折算 CNY。
- `v1.0.4`：引入 `sync/bootstrap` 增量刷新；休市冻结口径统一（Flutter/Web/后端）。
- `v1.0.3`：收益日历口径修复；分市场明细与历史回填（含 `market_breakdown`）。
