...
继续增强智谱截图识别：复杂整页持仓图改成“先识别、再转写兜底、最后本地拆行”，并补上限流重试，尽量减少空结果和直接报错。

### 主要变化
- **OCR 专用配置补出文本模型回退**：后端 [kona_tool/ai_handlers.py](/Users/kona/Desktop/kaka/kona_repo/kona_tool/ai_handlers.py) 现在在 OCR 专用模型覆盖时，会保留供应商原来的文本模型，避免复杂截图转写后的第二步整理还继续硬用视觉模型。
- **复杂截图兜底改成“转写 + 本地拆行”**：后端 [kona_tool/core/portfolio_ocr.py](/Users/kona/Desktop/kaka/kona_repo/kona_tool/core/portfolio_ocr.py) 不再死磕“一次就出 JSON”，而是先尝试普通提取；失败后改为让模型只盯资产列表区域逐行转写，再由本地代码拆出 `名称 / 代码 / 数量 / 成本价`，减少二次结构化空返回。
- **整页长列表补上区域兜底**：对银河证券这类深色整页持仓图，后端现在会在全图转写失败时继续尝试“上半段 / 中间区域 / 下半段”资产区，避免模型盯整页时完全不给结果。
- **上游限流自动重试**：智谱返回 `429 / 1302 / 1305` 这类访问量过大错误时，后端会自动重试，不再一次拥堵就直接失败。
- **复杂图结果容量放宽到 12 条**：后端不再把复杂截图结果硬截成 5 条，最多可返回 12 条，避免整页持仓图刚识别出几条就被自己裁掉。
- **补齐复杂截图回归测试**：后端 [kona_tool/tests/test_portfolio_api.py](/Users/kona/Desktop/kaka/kona_repo/kona_tool/tests/test_portfolio_api.py) 新增“转写兜底”“两行一条资产拆行”“最多 12 条”“名称尾部脏符号清理”等测试，避免后面再回退。

### 影响范围
- 后端：`/api/portfolio/ocr_parse_asset` 的复杂截图识别链路
- 线上识别：银河证券、富途这类整页深色持仓图的成功率和稳定性

### 验收重点
- 桌面测试图 [测试1.jpg](/Users/kona/Desktop/测试1.jpg) 这类整页深色持仓图，应至少能提取出多条资产草稿，不应再一上来直接空结果
