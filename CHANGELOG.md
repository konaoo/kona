...
### 这版一句话
全面治理后端 API 性能瓶颈：汇率、基金净值日期、行情价格三层缓存全面升级，消除收盘后无谓的外网请求，写操作和首屏加载速度从秒级提升到毫秒级。

### 主要变化
- **汇率缓存从 60 秒延长到 10 分钟**：`core/price.py` 中 `get_forex_rates()` 的缓存 TTL 从 60 秒延长到 600 秒，stale fallback 从 5 分钟延长到 30 分钟。汇率日内波动极小，10 分钟完全足够。
- **基金净值日期新增 5 分钟缓存**：`core/fund.py` 中 `get_fund_latest_nav_date()` 新增 300 秒 TTL 内存缓存，消除每次请求 4 只基金串行外网调用的 540ms 瓶颈。净值确认日期一天最多变化一次。
- **行情价格缓存收盘后自动延长到 1 小时**：`core/price.py` 中 `PriceCache.get()` 新增 `ttl_override` 参数，`batch_get_prices` 和 `batch_get_prices_fast` 根据每只股票所属市场的开盘状态动态调整缓存 TTL——开盘中 60 秒，收盘后 3600 秒。不同市场（A 股 15:00 收 / 港股 16:00 收 / 美股 4:00 收）各自独立判断。

### 影响范围
- 后端：`core/price.py`（汇率缓存 + 行情缓存动态 TTL）、`core/fund.py`（基金净值日期缓存）
- 全平台受益：Flutter App 和 Web 端共享同一套后端 API

### 验收重点
- 收盘后打开 App 或 Web，行情请求应直接命中缓存（日志中 `quote.batch.base` < 3ms），不再发外网请求
- 买入/卖出/添加资产等写操作应秒回，不再有 3-5 秒卡顿
- 开盘时行情仍每 60 秒刷新一次，不受影响

## 2026-03-25-01

### 这版一句话
完成实时盈亏与快照分层重构（包括补齐缺失的独立作用域模块文件），彻底解决多账本与全局数据计算脱节问题，并修复了超时降级等高危防护缺陷。同时修复了场内 ETF 收益被场外基金晚到净值覆写丢失的 Bug。

### 主要变化
- **快照架构重构**：重构了 `take_snapshot` 链路，全局快照改为直接由底层各个子账本的数据汇总生成，消灭了全局与子账本分别计算导致的金额分叉问题。
- **账本隔离与日历优化**：前端通过明确的范围 Store 取代了旧传参，确保了多账本数据的严格隔离；并恢复了网页端日历视图当天的实时跳动与覆盖拼接。
- **实时熔断保护补齐**：在分析页接口中重新加入了由于大批量拉取行情超时引起的 fallback 数据库降级保护，阻止前端因外部接口缓慢而触发白屏 500。
- **场内 ETF 与场外基金拆分**：底层的 `market_from_asset` 逻辑现在会将所有的场内交易型基金（如 51xxxx、15xxxx 等）直接合并到 A 股大盘市场进行归口统计和展示，彻底杜绝了之前场内 ETF 的日结收益被场外基金 T+1 日结推平覆写冲消的严重问题。
...
### 验收重点
- 在多账本之间来回切换，确认各自的实时盈亏没有互相串走。
- 确认前端在网络极差或行情源无响应时，页面最上面能够显示保底历史盈亏而不报错。
- 由于重构了收口并且实施了线下历史修复，收益日历前两天的亏损不会再由于基金到账的覆盖而莫名其妙消失变正了。

## 2026-03-24-08

### 这版一句话

修复了 App 端收益日历数据不刷新的问题，统一了 Web 端全站 Logo 和视觉边框样式，并解决了 Web 端日历当天不显示实时盈亏的缺陷。

### 主要变化
- **后端账本快照（Ledger Snapshot）同步增强**：发现了导致 App 端数据与 Web 不一致的深层原因——账本快照表未能同步基金晚到结算及盘前剥离逻辑。本次重构了快照写入链路，确保 `ledger_daily_snapshots` 与全局 `daily_snapshots` 共享同一套收益修正逻辑，从根源上杜绝了数据分叉。
- **Flutter 收益日历缓存 TTL 机制**：给 Flutter 分析页日历和概览缓存增加了 10 分钟 TTL 过期机制（SWR 策略），解决了 App 端因为长期显示旧缓存导致与 Web 端数据不一致的问题。
- **Web 全站 Logo 统一**：将门户页、页脚、侧边栏及各功能页 Logo 统一替换为官方 `logo.svg`。
- **Web 视觉降噪**：移除了 NavBar 和 TickerBar 底部的边框线，优化毛玻璃视觉融合度。
- **Web 日历当天实时显示**：修复了 Web 端查看全局视图时，当天的日历格无法被盘中实时盈亏覆盖的问题（底层原因是匹配日期的文本格式没对齐），现在今天那一格的数据也准了。

### 影响范围
- Flutter：分析页、收益日历
- Web：全站布局（导航栏、页脚、各页面 Logo）、全局视图收益日历

### 验收重点
- 打开 App 分析页，确认收益日历数据能在 10 分钟内同步服务端修正后的最新值（无需手动下拉）。
- 刷新 Web 页面，确认导航栏底部和跑马灯底部没有了细线边框。
- 确认 Web 页脚的 Logo 已经从手绘 CSS 改成了真实的 SVG 图片。
- 在盘中时间查看 Web 端全局视角的收益日历，当天的格子应当和顶部的“当日总计盈亏”保持同步一致。

## 2026-03-24-07

### 这版一句话

根据反馈，已经将投资页“持仓明细”板块的「添加资产」及「视图切换」按钮整体下移，与资产类别过滤标签（全部、A股等）排列在同一水平线上。

### 主要变化
- **调整按钮位置**：修改了 `AppInvestPage.vue` 中的 HTML 结构，将原来位于“持仓明细”标题所在行的右侧操作区，完整迁移到下方的分类标签过滤器所在的同行同行右侧。
- **改善自适应布局**：为调整后的新按钮外层容器添加了 `flex-wrap: wrap` 支持，确保在较小屏幕上按钮和分类标签不会发生重叠变形。

### 影响范围
- 投资页的“持仓明细”区块。

### 验收重点
- 进入投资页，滑动到持仓明细，确认标题所在行只有“持仓明细”四个字。
- 确认“添加资产”按钮以及“卡片/列表列表”切换按钮并排显示在了“全部、A股、港股、美股、基金”分类标签同一行的最右侧。

## 2026-03-24-06

### 这版一句话

电脑端投资页面的分类（全部、A股、港股等）排版太宽已修复，将其约束在合理宽度内并和首页保持一致；另外优化了这些分类标签的展现顺序。

### 主要变化
- **修正投资页宽度**：缩小了 `AppInvestPage.vue` 电脑端的最大容器限制（从 `1200px` 调整到 `1040px`），并对内部标签拦增加了 `width: fit-content` 以避免因为 flex 特性导致排版在宽屏上左右两端撑得过开。
- **调整分类标签顺序**：按照使用习惯，将资产分类的标签顺序从 `全部、港股、美股、A股、基金` 统一调整为了 `全部、A股、港股、美股、基金`。

### 影响范围
- 投资页（电脑端及移动端水平滚动区域）

### 验收重点
- 打开电脑端访问“投资”页，确认整个展示区的宽度与“首页”保持一致，分类标签不再填满屏幕。
- 确认分类按钮从左到右依次是“全部、A股、港股、美股、基金”。

## 2026-03-24-05

### 这版一句话

这版主要在首页加上了全局隐私模式的切换图标，同时修复了因为去掉了标题栏导致 GitHub Actions 流水线自动化测试报错的问题。

### 主要变化
- **首页补回隐私小黑眼**：由于去掉了移动端的顶部标题栏，在首页右上角的总资产旁边，重新加回了 `👁️`/`🙈` 隐私切换按钮，现在手机端也能一键隐藏敏感资产数值。
- **修复自动化测试报错**：去掉了顶部标题栏后，原先渲染的 `我的资产` 文字找不到了，导致 CI 自动化测试红叉连篇。现在改成校验卡片上一定存在的 `总资产` 字段，解决了流水线的失败。

### 影响范围
- 首页移动端的总资产卡片区
- 项目流水线（E2E 测试用例 `smoke.spec.ts`）

### 验收重点
- 在手机端能看到首页总资产栏右侧出现小眼睛，点击后数字会被隐藏。
- 确认推送上 Github 后，Action 流水线顺利点亮绿勾。

## 2026-03-24-04

### 这版一句话

这版主要修复了 Web 端在手机上的各种样式错乱和接口连不上等问题，包括去掉顶部标题栏、优化投资页和分析页的展示以及快讯分类按钮的排版。

### 主要变化
- **去掉全站标题栏**：由于占据了屏幕顶部太多空间，现在移除了所有 Web 端页面公共的标题栏（`AppShell` 中），页面看起来变得更宽敞自然，同时也调整了下方的留白。
- **缩小投资与分析大卡片**：投资页的资产卡片字号、内边距，以及分析页的盈亏概览面板，现在在移动端（`max-width: 640px`）都会按比例自动缩小，防截断也更协调。
- **修复快讯分类防遮挡**：修复了快讯页面分类栏里“★ 仅看重要”按钮在右侧被意外截断的问题，改成当在手机屏幕上放不下时自然折行到下一行，不再强制在一行里挤压。
- **解禁本地快讯接口**：修复了本地开发环境打开快讯页没有数据的问题（通过直接修改数据库 `portfolio.db` 的 `admin_api_policies` 表重新启用了被禁封的 `api.news` 策略）。

### 影响范围
- 所有 Web 端页面的整体布局 (`.page` 高度计算)
- 投资页顶部卡片展示、分析页日历及概览展示、快讯页顶部过滤栏

### 验收重点
- 在手机端刷新这几个页面，看看排版是否清晰自然，重点按钮（例如“仅看重要”）是否不再被遮住
- 从电脑打开 Web 时，这些针对手机特供的样式不受影响
- 访问快讯页能够刷出最新的文章，不再是一片空白
...
- **Flutter 汇总和本地乐观态一起收口**：同一轮里补上了两个容易被忽略的边角：一是首页/分析页的 `investDayPnl` 汇总只会累计“当前应该计入当日口径”的持仓，不会把休市但允许单条显示冻结值的资产也算进总额；二是本地乐观持仓如果出现无效负价格，不会再把投资总市值和持仓盈亏率算成负数。
...
把“历史用户根本没有默认账本，导致账本历史补不回来”的迁移缺口补上：后端现在会正式给有投资历史的老用户补默认账本，并同步补齐可明确归属的旧历史快照。

### 主要变化
- **默认账本补齐不再只发生在最早那次迁移**：后端 [kona_tool/core/db_schema.py](/Users/kona/Desktop/kaka/kona_repo/kona_tool/core/db_schema.py) 现在每次初始化都会扫描投资相关历史数据来源，为缺失默认账本的用户补创建默认账本，不再只在 `portfolio` 第一次加 `ledger_id` 时顺手补一次。
- **快照型老用户也会被纳入补账本范围**：这次把 `daily_snapshots` 和 `ledger_daily_snapshots` 也纳入账本用户来源。像那批只有历史快照、没有当前持仓和交易的老用户，以前会被彻底漏掉；现在也会拿到默认账本，并继续走默认账本历史回填。
- **补齐“只有快照历史”的回归测试**：后端 [kona_tool/tests/test_database_schema.py](/Users/kona/Desktop/kaka/kona_repo/kona_tool/tests/test_database_schema.py) 新增“只有 `daily_snapshots`、没有持仓和交易的用户，也会在初始化时自动获得默认账本并补回账本历史快照”的测试，锁住这次线上缺口。

### 影响范围
- 后端：账本初始化、默认账本补齐、默认账本历史快照回填
- 线上历史用户：没有默认账本但已有投资历史的账号

### 验收重点
- 线上缺默认账本的历史用户，部署后应自动补出默认账本
- 这些用户的默认账本历史快照应与旧全局快照对齐
- 已经有默认账本的用户，不应被重复创建或改乱
...
继续收口 Flutter 账本管理页交互：新增和删除账本改成先在本地立即反馈，再由后端静默补写，减少 3 到 5 秒的等待感。

### 主要变化
- **新增账本改成乐观更新**：Flutter [flutter/lib/providers/app_state.dart](/Users/kona/Desktop/kaka/kona_repo/flutter/lib/providers/app_state.dart) 现在会先把新账本以临时项插进本地状态，成功后再替换成真实 `ledger_id`，失败才回滚，不再等接口和二次拉列表后才显示。
- **删除账本改成先本地移除**：同一文件现在会先把目标账本从本地状态里移掉，删除当前账本时首页刷新也改成后台跑，用户不再被整条刷新链路卡住。
- **管理页去掉重复拉取和整页锁死**：Flutter [flutter/lib/pages/ledger_management_page.dart](/Users/kona/Desktop/kaka/kona_repo/flutter/lib/pages/ledger_management_page.dart) 现在不再在成功后手动再拉一次账本列表，也不再用整页 `IgnorePointer` 把界面锁死。
- **新增中的账本显示轻状态**：同一页面里，刚创建但还在等待接口确认的账本会显示一个很轻的“创建中”状态，平时列表直接使用 `AppState.ledgers`，只有拖动排序时才切到本地副本，避免出现“后端已成功但列表自己跳回去”的竞态。

### 影响范围
- Flutter：投资页里的“管理账本”页面
- Flutter：账本新增、删除、排序时的即时反馈和等待体验

### 验收重点
- 新增账本后，列表里应立刻出现新账本，不应等 2 到 5 秒后才显示
- 删除空账本后，列表里应立刻消失，不应长时间停在原地等待
- 新增失败时，临时账本应回滚并给出失败提示
- 排序时仍应保持拖动中的本地顺序，不被后台列表瞬间打乱
...
- **线上 `kona` 数据收口**：恢复 `2026-03-19` 的原始历史拆分，保留 `2026-03-20 = -1611.46` 的最终值，同时继续保持 `2026-03-21 = 0`，让分析页顶部概览、收益日历和快照表重新一致。
...
### 验收重点
- 分析页顶部 `当日总计盈亏` 和收益日历 `3/20` 应同时显示 `-1611.46`
...
把 App 登录页补成真正的浅色模式，并修掉登录/注册切换时一闪而过的灰色阴影。

### 主要变化
- [flutter/lib/pages/login_page.dart](/Users/kona/Desktop/kaka/kona_repo/flutter/lib/pages/login_page.dart)：登录页改成按当前主题切换浅色/深色两套配色，页面背景、卡片、输入框、弹窗、提示条和系统状态栏不再固定深色。
- [flutter/lib/pages/login_page.dart](/Users/kona/Desktop/kaka/kona_repo/flutter/lib/pages/login_page.dart)：登录/注册切换按钮在浅色模式下取消背景过渡动画，避免切换时短暂闪出一层灰色阴影。
- 登录页局部文字样式和输入框边框统一走同一套登录页色板，后面再调浅色视觉时不会散着改。
