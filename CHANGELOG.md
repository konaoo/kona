...
### 主要变化
- **Flutter 收益日历缓存 TTL 机制**：由于基金净值晚到回写会修正历史 `day_pnl`，本次给 Flutter 分析页日历和概览缓存增加了 10 分钟 TTL 过期机制（SWR 策略），解决了 App 端因为长期显示旧缓存导致与 Web 端数据不一致的问题。
- **Web 全站 Logo 统一**：将门户页、页脚、侧边栏及各功能页的 Logo 统一替换为官方 `logo.svg`，保持品牌基因一致。
- **Web 视觉降噪**：移除了导航栏（NavBar）和跑马灯（TickerBar）底部的双横线边框，使其与背景毛玻璃效果更自然地融合。
