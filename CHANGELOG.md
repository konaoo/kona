...
### 这版一句话
Web 和 App 的收益日历支持用左右箭头按月切换，当前月不再显示“下一个月”。

### 主要变化
- 日视图的年月选择器两侧补齐月份切换箭头，可直接查看前一个或后一个已有收益数据的月份。
- 到当前月时隐藏右侧箭头，避免用户误切到尚未开始的未来月份。
- App 在最早可查看月份禁用左侧箭头；Web 在没有更早可查看月份时不显示该箭头。
- 新增 Web 与 Flutter 回归测试，覆盖前后切月和当前月边界。
- Gradle 包装器升级到 `8.14`，适配 GitHub 当前 Flutter 构建环境，恢复调试 APK 构建门禁。

### 影响范围
- 只影响 Web 和 App 分析页的收益日历切月操作。
- 不改收益金额、持仓、交易记录、快照或任何线上数据。
- 不改变 App 功能，仅修复自动构建环境兼容性。

### 验收重点
- 查看历史月份时，可通过箭头前后切换到相邻已有月份。
- 查看当前月时，右侧“下一个月”箭头不显示。

## 2026-07-28-01

### 这版一句话
统一分析页交易日内的实时收益口径，避免本月总计、年度日历和当日收益显示成不同数字。

### 主要变化
- 实时行情已经属于今天时，用“实时当天收益减去今天快照收益”的差额，同时更新当天日历、本月总计、今年总计和当前年份日历当月卡片。
- 全部累计盈亏在实时接口提供累计值时也同步采用该值，避免总览和实时持仓相互矛盾。
- 历史月份、历史年份、实时行情未就绪或取价失败时，继续只显示已保存快照，不回写数据也不猜历史数值。

### 影响范围
- 影响 Web 和 App 共用的分析页总览与收益日历接口。
- 不修改持仓、交易记录、成本价、历史快照或线上用户数据。

### 验收重点
- 交易日内查看当前月份时，“本月总计”和日历底部“当月累计”应一致；当前年份日历中的当月卡片也应一致。
- 切换到过去月份或年份后，数字应继续保持历史快照，不随当天行情变化。

## 2026-07-21-01

### 这版一句话
新增场外基金历史净值自动对账，避免净值晚到后，收益日历长期少一条基金收益。

### 主要变化
- 定时任务每天按 T+1 回扫最近 2 个已结束日期，只对缺少基金明细的资产用相邻两个已确认的历史净值计算日收益；最近 7 天和人工全面核验可单独启用。
- 缺少明细、收益为错误的 `0`、或金额与确认净值不一致时才补；确认净值没变时，也会明确保存一条 `0` 收益明细。
- 场内 ETF、股票和当天数据不参与；外币基金无法从历史明细可靠倒推出当日汇率时直接跳过，不用今天汇率猜历史。
- 实际写库前通过 SQLite 备份 API 自动生成一致性备份；写入后从账本明细重新汇总用户全局日历。

### 影响范围
- 自动任务只影响收益日历的基金逐资产明细、基金市场收益和当天收益总额。
- 不改持仓数量、成本价、交易记录、实时价格或累计收益。

### 验收重点
- 净值晚一天发布后，下一次对账应补齐对应日期的基金收益明细。
- 净值不变的交易日，日历点开后仍能看到该基金，收益为 `0`。

## 2026-07-20-03

### 这版一句话
修复基金日历把“取价失败”误当成“收益为 0”的问题，并补齐 kona 已核实的历史基金日收益。

### 主要变化
- 场外基金只有在已拿到确认净值、且前后净值相同时，才会在收益日历中保存一条 `0` 收益明细；取价失败或没有确认净值日时不再伪造 `0`。
- 汇丰贝莱德环球收益基金（`ft_LU1116320737`）历史净值改用已与贝莱德官网逐日核验一致的 Morningstar 日净值接口，Fidelity 保留为备用。
- 按官方历史净值回填 kona 账本 62 的易方达增强回报债券B 14 天、汇丰贝莱德环球收益基金 3 天日收益；其中易方达有 9 条旧记录原本错误显示为 `0`，本次一并修正。
- 17 个日期的基金市场收益和当天总收益同步更新，合计增加 `1,682.56` 元历史日收益。

### 影响范围
- 线上数据只修改 kona 的账本 62 与同一用户的历史快照明细、市场拆分和日收益汇总。
- 不改持仓数量、成本价、交易记录、实时价格或当前累计收益。
- 后续所有用户的场外基金快照都会区分“确认不变”和“暂时取价失败”。

### 验收重点
- 打开 kona 的 4 月 15 日、6 月 9 日、6 月 30 日、7 月 15 日收益日历，应能看到对应基金的真实日收益。
- 确认净值不变的基金日期仍显示 `0`；短暂取价失败的日期不再提前显示假 `0`。

## 2026-07-20-01

### 这版一句话
修复收益日历的基金日收益缺失，并保证无涨跌的持仓也会在日历明细中显示为 0。

### 主要变化
- 已按易方达增强回报B（`f_110018`）的官方历史净值，定向补齐 kona 的账本 62 在 7 月 2 日至 16 日缺失的 9 天日收益。
- 同步修正这 9 天的基金市场收益和日历当天总收益；不改持仓、成本价或当前累计收益。
- 基金净值晚到后回填历史日历时，不再误删同一天仍持有但净值未变的基金 0 收益明细。
- 增加回归测试，防止后续同类回填再次把 0 收益资产从日历中删掉。

### 影响范围
- 线上数据只修改 kona 的账本 62、易方达增强回报B 的 9 个历史日期。
- 后续所有用户的收益日历明细都会保留仍持有资产的 0 收益行。
- 不影响股票和基金的持仓数量、成本、交易记录和当前累计收益。

### 验收重点
- 打开 kona 的 7 月 2 日、3 日、6 日、7 日、10 日、13 日至 16 日日历，应能看到易方达增强回报B 的对应日收益。
- 后续基金净值不变的交易日，日历明细仍应显示该基金，收益为 0。

## 2026-07-20-02

### 这版一句话
按可核实的历史净值，补齐 kona 账本 62 在 4 至 6 月遗漏的基金日收益和 0 收益明细。

### 主要变化
- 易方达增强回报B补齐 41 个可核实净值日，日收益合计调整为 `-6,760.64`。
- 摩根亚洲股息补齐 2 个可核实净值日，日收益合计调整为 `-997.49`。
- 摩根亚洲总收益补齐 17 个可核实净值日，其中 15 天是应展示的 0 收益；两笔实际涨跌相互抵消。
- 汇丰贝莱德基金因历史净值来源拒绝访问，本次不写入任何估算数据。

### 影响范围
- 线上数据只修改 kona 的账本 62、2026 年 4 月至 6 月的已核实基金日历数据。
- 共新增 60 条逐资产明细，日历总收益合计减少 `7,758.13`。
- 不改持仓、成本价、交易记录、当前累计收益或服务代码。

### 验收重点
- 打开上述日期的收益日历，应能看到对应基金收益；无净值变化的日期显示为 0。
- 4 月、5 月、6 月日历的逐资产明细、基金市场收益和当天总收益应保持一致。

## 2026-07-14-01

### 这版一句话
Web 资产修正弹窗补齐并同步 App 端“备注”输入和保存功能。

### 主要变化
- 修正弹窗增加备注：在 Web 端的 `InvestTradeModal`（资产交易/修正弹窗）的“修正/调整”面板下，补齐了“备注 (可选)”输入框。并支持在保存各项修正（如成本价修正、数量修正、分红记录、手续费记录等）时，将备注内容同步发送并保存至后端数据库。
- 备注框样式对齐：移除了备注输入框上覆盖的内联 padding 样式，使其完美继承系统全局的 `.num-input` 规范高度与样式，与上方的价格/数量输入框保持视觉一致。
...
### 这版一句话
修复资产交易弹窗“成本修正”输入价格与详情页最终展示价格不一致的口径 Bug。

### 主要变化
- 成本修正口径对齐：在 `InvestTradeModal` 弹窗组件中，移除保存成本价修正时在 `price` 属性上自作聪明反向折算均摊调整额（`currentAdj / holdingQty`）的公式。直接将用户输入的价格作为 `price` 发送并保存给后端。同时，在打开弹窗时的目标成本价默认值，从包含分红拉低后的 `currentDisplayCostPrice` 变更为与详情页完全一致的原始成本价 `currentRawCostPrice`，彻底实现“输入多少就显示多少”的直觉对齐。
...
### 验收重点
- 访问默认账本下的资产详情页，交易记录加载应为 200 成功，不应报 400 错。
- 点击“加仓”，资金账户下拉选择框应能够正常渲染包含“招商银行”等在内的全部可用资金账户。
- 在网络断开或 token 刷新临界点，下拉选择框至少要能渲染出“外部资金/初始转入”及“添加资金账户”选项，不能出现空白。

## 2026-07-13-01

### 这版一句话
Web 资产详情页的交易记录改为和 App 使用同一套投资交易记录口径。

### 主要变化
- Web 资产详情页不再从全站交易流水里截取最近记录
- Web 改用 `/api/portfolio/transactions` 读取当前持仓的完整交易记录
- 查询时带上当前持仓所在账本，避免跨账本记录混在一起
- Web 交易记录展示补齐分红、手续费、税金、成本修正、数量修正、持仓修正等记录
- Web 交易记录样式从表格改成和 App 更接近的“类型、金额、明细、时间”结构

### 影响范围
- Web 端资产详情页交易记录区域
- 不改变 App 端逻辑
- 不改变后端交易记录接口、收益、快照、持仓成本和行情价格口径

### 验收重点
- 工商银行这类持仓应在 Web 端看到分红、手续费和成本修正记录
- Web 端交易记录数量和类型应与 App 端对齐
- 加仓记录不应再显示一排“盈亏 0”造成误解
- Web 构建应通过

## 2026-07-09-01

### 这版一句话
统一港股代码、价格缓存和行情源口径，避免同一只港股因为写法或旧行情源不同显示出不同现价。

### 主要变化
- 港股 `700.HK`、`0700.HK`、`00700.HK`、`hk700`、`hk00700` 统一按 `hk00700` 取价
- 港股入库解析统一补齐为 5 位代码，例如 `0700.HK` 会保存为 `00700.HK`
- OCR 导入港股候选代码也统一补齐 5 位
- 价格缓存按标准代码存取，避免同一只股票被拆成多份缓存
- 港股现价优先使用东财行情，腾讯和新浪作为备用源
- 腾讯港股行情在开盘时如果返回明显过期的时间戳，会被当作不可用并继续回退
- 东财 A/HK 行情改用 Python 可稳定访问的入口和请求头
- 补充港股代码标准化、缓存复用、行情源顺序和过期行情判断测试

### 影响范围
- 港股现价接口和批量现价接口
- 持仓新增、OCR 导入时的港股代码解析
- 价格缓存命中逻辑
- 不改变收益、快照、持仓数量、成本价和汇率计算口径

### 验收重点
- 腾讯控股不同代码写法应拿到同一条港股行情链路
- `0700.HK` 不应再被错误当成 `hk0700`
- 港股盘中腾讯免费源明显滞后时，不应挡住东财新行情
- A股、美股、基金和 ISIN 基金代码不应受影响
- 后端全量测试应通过

## 2026-07-08-03

### 这版一句话
修复普通资产改名和删除时“没改到也像成功”的问题。

### 主要变化
- 后端更新/删除现金、其他资产、负债时，会检查真实影响行数
- 如果资产不存在、已被删除或不属于当前用户，接口返回 404 和 `ASSET_NOT_FOUND`
- Web 首页资产弹窗保存失败时不再关闭弹窗
- Web 删除资产失败时会提示原因，不再让用户误以为已经删除
- Web 保存和删除按钮增加处理中状态，避免重复点击
- 后端投资相关测试会清理接口策略计数和测试配置，避免全量 unittest 被上一个用例误伤成 429

### 影响范围
- Web 首页现金、其他资产、负债的新增、编辑、删除弹窗
- `/api/cash_assets/update`、`/api/other_assets/update`、`/api/liabilities/update`
- `/api/cash_assets/delete`、`/api/other_assets/delete`、`/api/liabilities/delete`
- 不改变投资持仓、收益快照和资产金额计算口径

### 验收重点
- 删除不存在的普通资产应提示失败，不应显示成成功
- 修改不存在的普通资产应提示失败，不应关闭弹窗
- 正常修改名称、金额、币种后仍应刷新首页资产列表
- 正常删除资产后仍应刷新首页资产列表
- 后端全量测试不应再因为测试间限流计数残留而红叉

## 2026-07-08-02

### 这版一句话
修复资产调整保存口径不一致、前端提前提示成功，以及测试限流偶发红叉的问题。

### 主要变化
- 线上已清理 `tuanzi` 用户一条金额为 0 的历史其他资产记录
- Web 编辑现金/其他资产/负债时，金额、名称、币种改为一次提交，避免只保存了一半
- App 资产调整弹窗改为等服务器保存成功后再提示“已记录”和关闭弹窗
- App 和 Web 金额校验统一：现金可以为 0，其他资产和负债不能直接修正为 0
- 后端资产调整接口支持一次保存余额、名称和币种；业务拒绝改为 400，不再误报 500
- 后端测试环境会清理内存限流计数，避免全量测试被上一个用例误伤
- Web lint 配置改为当前依赖可用的规则组合，恢复 lint 命令可执行

### 影响范围
- Web 首页资产编辑弹窗
- App 资产新增/调整弹窗
- `/api/assets/<type>/<id>/adjustments` 资产调整接口
- 后端测试和 Web lint 检查
- 不改变投资持仓、收益快照和行情价格口径

### 验收重点
- 修改资产金额、名称、币种时，要么一起成功，要么一起失败
- App 保存失败时不应提前关闭弹窗或提示成功
- 非现金资产不能通过“修正余额”保存成 0
- 负债通过“减少”还到 0 后仍应自动从列表消失
- Web build/test/lint、Flutter analyze/test、后端全量测试应通过

## 2026-07-08-01

### 这版一句话
修复负债通过“减少余额”还到 0 后仍继续显示的问题。

### 主要变化
- 线上已清理 `kona` 用户一条金额为 0 的历史负债记录
- 非现金资产通过调整记录扣到 0 时，后端会保留调整流水并删除资产记录
- 非现金资产通过调整记录扣成负数时，后端会拒绝本次调整
- 补充负债扣到 0 自动删除、扣成负数拒绝的后端测试

### 影响范围
- Web / App 首页的现金、其他资产、负债列表
- `/api/assets/<type>/<id>/adjustments` 资产调整接口
- 现金资产仍允许为 0，不受这次规则影响

### 验收重点
- `kona` 用户不应再看到 `万科房贷` 这条 0 负债
- 负债减少到 0 后应从负债列表消失
- 负债减少超过当前余额应被拒绝
- 资产账户和投资交易相关后端测试应通过
...
- 管理后台用户管理页
- 管理员重置用户密码的前端交互
- 不改变后端重置密码接口、临时密码规则和登录失效规则
- GitHub Actions Web 验收和 Web 自动部署链路
...
### 这版一句话
修复价格健康检查把备用行情源失败误报成生产告警的问题。

### 主要变化
- 价格健康告警默认只检查关键实时价格源和汇率源
- Yahoo、Nasdaq、FT、MarketScreener、东财手机端、天天基金等备用或历史源失败不再单独触发生产告警
...
### 这版一句话
修复新服务器价格异常巡检日报因为报价返回值变更而失败的问题。

### 主要变化
- 后台价格巡检统一兼容 4 位和 5 位报价返回结果
- 巡检来源明细保留价格日期，方便后续排查旧价或缓存价
- 补充后台价格巡检回归测试，防止定时日报再次因为拆包失败中断

### 影响范围
- 新服务器每天 `08:20` 的价格异常巡检日报
- 管理后台价格探测接口的来源明细

### 验收重点
- `scripts/run_price_alert_report.py` 应能正常跑完
- 后台接口相关测试应通过
- 线上 `/var/log/kona_price_alert_report.log` 不应再出现 `too many values to unpack`

## 2026-05-25-03

### 这版一句话
修复 Web 首页 `kona` 用户现金资产、其他资产、负债和资产趋势可能显示为 0 的问题。

### 主要变化
- Web 首页改为通过 `/api/sync/bootstrap` 一次拉取现金资产、其他资产、负债和历史快照
- 保留原有分散接口作为同步接口失败时的兜底
- `sync/bootstrap` 的历史快照范围从一年扩到完整历史范围，和 `/api/history` 保持一致
- 补充 Web 首页状态单测和后端同步历史范围测试

### 影响范围
- Web 首页资产总览
- Web 首页资产折线图
- 后端同步引导接口里的 `history` 数据范围

### 验收重点
- `kona` 登录 Web 首页后，现金资产、其他资产、负债不应再显示为 0
- 首页资产折线图应能使用真实历史快照显示
- Web 首页单测、Web 构建、后端同步接口测试应通过

## 2026-05-25-02

### 这版一句话
修复迁移后部分旧客户端或旧缓存入口首页资产趋势不显示的问题。

### 主要变化
- `/api/history` 默认返回完整历史范围，兼容没有传 `days` 的旧客户端
- Flutter 历史接口常量明确请求 `days=5000`
- Web 首页历史曲线成功加载后写入本地缓存，短时网络失败时优先用已有历史曲线兜底
- 补充历史接口默认范围和 Flutter 历史接口配置测试
...
### 验收重点
- 旧客户端调用 `/api/history` 不应只拿一年默认范围
- Web 首页历史接口短时失败时不应直接清空已有趋势图
- 后端历史接口测试、Web 构建、Flutter 配置测试应通过
...
### 这版一句话
发布 Flutter 客户端 `1.0.34`，正式切到 `kakalog.fun` 域名访问。

### 主要变化
- Flutter 客户端版本号从 `1.0.33` 升级到 `1.0.34`
- App 默认 API 地址从新服务器 IP 改为 `https://kakalog.fun`
- 生产默认关闭明文 HTTP 兼容，改走 HTTPS 域名
- 后端默认客户端更新版本同步提升到 `1.0.34`
...
### 验收重点
- 已安装 `1.0.33` 的 Android 用户应能安装 `1.0.34`
- `/api/app/version` 应返回 `version=1.0.34`、`buildNumber=34`
- 新 APK 登录和主要数据接口应访问 `https://kakalog.fun`
- 域名 HTTPS 健康检查应保持正常

## 2026-05-21-02

### 这版一句话
发布 Flutter 客户端 `1.0.33`，让新安装包默认连接迁移后的新服务器。

### 主要变化
- Flutter 客户端版本号从 `1.0.32` 升级到 `1.0.33`
- 后端默认客户端更新版本同步提升到 `1.0.33`
- 新 APK 默认 API 地址使用 `http://114.132.246.156`
...
### 验收重点
- 已安装 `1.0.32` 的 Android 用户应能安装 `1.0.33`
- `/api/app/version` 应返回 `version=1.0.33`、`buildNumber=33`
- 新 APK 登录和主要数据接口应访问新服务器 IP

## 2026-05-21-01

### 这版一句话
完成腾讯云生产服务器从旧实例迁移到新实例。

### 主要变化
- 新生产入口改为 `http://114.132.246.156`
- GitHub Actions 部署目标 `SSH_HOST` 已切到新服务器
- Flutter 默认生产 API 地址同步改为新服务器 IP
- Flutter 测试断言和生成客户端默认地址同步改为新服务器 IP
- 旧服务器 `114.132.238.12` 文档引用已同步更新，后续运维默认按新机处理
- 备份和快照校验 timer 修正为北京时间早上执行，避免按晚上时间误跑

### 影响范围
- 线上后端、Web 静态入口、定时任务、数据库备份任务
- GitHub Actions 后续部署目标
- 下一次 Flutter 发包时的默认 API 地址

### 验收重点
- 新 IP `/health`、`/app/login`、`/admin/login` 返回 `200`
- 旧 IP 过渡转发到新机，避免旧客户端立刻断连
- 新机数据库完整性检查通过，用户数为 `183`
- 新机手工快照、备份、快照校验均已验证通过

## 2026-05-14-02

### 这版一句话
修复 GitHub Actions 中 Web 部署和后端重启并发时的误红问题。

### 主要变化
- Web 静态资源部署后的 smoke check 增加重试等待
- 避免后端服务正在重启的短暂窗口导致 Web 部署误判失败并回滚

### 影响范围
- GitHub Actions `Deploy Web`
- 生产部署脚本 `scripts/deploy/apply_web_artifact.sh`

### 验收重点
- 后端和 Web 同时部署时，Web smoke 遇到短暂 `5003` 连接失败应自动重试
- 服务恢复后 Web 部署应通过，不应误触发静态资源回滚

## 2026-05-14-01

### 这版一句话
修复当天卖出清仓时当日收益可能漏算的问题。

### 主要变化
- 当行情当前价与昨收价相同、无法从行情涨跌推导归属日时，若当天存在卖出流水且有真实昨收价，则按交易流水的市场本地日期归属当日收益
- 补充“当天清仓且价格平盘仍计入卖出收益”的后端回归测试

### 影响范围
- App / Web 共用后端：投资页当日收益、收益市场拆分、分析页依赖当日收益的统计
- 已清仓持仓不重新出现在持仓列表，但当天卖出收益会继续计入当日总收益

### 验收重点
- A 股盘中卖出清仓后，投资页当日总收益不应少掉该股票卖出前已产生的当日收益
- 当前价等于昨收价时，卖出成交价高于昨收价的清仓收益仍应计入当天

## 2026-05-13-02

### 这版一句话
修复后台无时区时间被多加 8 小时展示的问题。

### 主要变化
- 前端公共 `shortDateTime` 对后端本地时间字符串改为本地格式化
- 带明确时区的时间仍统一转换为上海时间展示

### 影响范围
- Web / 后台：用户最近活跃、最近登录、缓存时间等短时间展示
- 数据库数据和 DAU 统计不受影响

### 验收重点
- 后台用户最近活跃不应把 `2026-05-13 18:xx` 展示成 `2026/05/14 02:xx`
- 带 `Z` 或 `+08:00` 的时间仍应正常显示为北京时间
...
### 主要变化
- 用户管理列表和移动卡片增加“重置密码”操作
- 重置成功后展示系统生成的临时密码，并提供复制按钮
- 临时密码弹窗关闭后不再保留密码，用户下次登录必须修改密码

### 影响范围
- Web 后台：用户管理页面
- 后端接口复用现有 `/api/admin/users/password/reset`

### 验收重点
- 点击重置密码后应二次确认，并在成功后展示临时密码
- 复制按钮应能复制临时密码
- 被重置用户旧登录状态应失效，下次登录必须修改密码

## 2026-05-11-01

### 这版一句话
统一分析页“全部”收益和投资页累计盈亏口径。

### 主要变化
- 分析页盈亏概览的“全部”改为读取最新快照的累计盈亏 `total_pnl`
- 月度、年度收益仍保持期间 `day_pnl` 累计口径
- 多账本筛选下的“全部”同步读取对应账本最新累计盈亏

### 影响范围
- Web / App 共用后端：`/api/analysis/overview?period=all`
- 分析页盈亏概览的“全部”金额和收益率

### 验收重点
- `kona` 用户分析页“全部”收益应和投资页累计盈亏一致，约为 `67500.34`
- 本月、本年收益仍按期间内每日收益累计展示
- 未来日期快照不应影响当前“全部”收益

## 2026-05-09-03

### 这版一句话
修复 Web 端非默认账本持仓修改成本价或数量时提示保存失败的问题。

### 主要变化
- Web 投资交易弹窗在修改成本价、数量、分红和手续费时会随请求带上 `ledger_id`
- 优先使用当前持仓自身的账本 id，缺失时再使用当前选中的账本 id
- 保持默认账本持仓的原有修改行为不变

### 影响范围
- Web：投资持仓调整弹窗
- 多账本用户：非默认账本里的持仓成本价、数量、分红和手续费调整

### 验收重点
- 非默认账本中的 `sz159659` 这类持仓修改成本价时不应再提示“保存失败，请重试”
- 默认账本持仓修改成本价仍应正常保存
- 修改后对应修正记录应写入正确账本
...
### 这版一句话
发布 Flutter 客户端 `1.0.32`，修复现金资产详情页删除资产后可能黑屏的问题。

### 主要变化
- 修复资产详情页删除成功后的自动返回逻辑，避免编辑弹窗和详情页同时触发返回导致空页面
- Flutter 客户端版本号从 `1.0.31` 升级到 `1.0.32`
- Android `versionCode` 随语义版本推导为 `32`

### 影响范围
- App：现金资产、其他资产、负债的单项详情页删除流程
- App 更新：旧版用户检查更新可识别 `1.0.32`

### 验收重点
- 在现金资产单项详情页进入编辑并删除后，应返回列表页，不应黑屏
- 已安装 `1.0.31` 的 Android 用户应能安装 `1.0.32`
- `/api/app/version` 应返回 `version=1.0.32`、`buildNumber=32`

## 2026-05-09-01

### 这版一句话
卖出后剩余市值低于 1 元人民币等值时，自动按清仓处理，避免基金小数尾差继续显示在持仓列表。

### 主要变化
- 后端卖出逻辑按 `剩余数量 * 卖出价 * 汇率 < 1 CNY` 判断自动清仓
- 普通卖出和卖出回款到现金账户都使用同一清仓判断
- 增加回归测试，覆盖纳指基金卖出后留下 `0.0037` 份尾差的场景

### 影响范围
- 后端：持仓卖出、卖出回款、持仓列表展示
- 用户数据：历史交易记录不变，只影响卖出后的当前持仓是否继续显示

### 验收重点
- 剩余市值低于 1 元人民币等值的持仓卖出后应从当前持仓列表消失
- 卖出交易流水、现金回款和撤销逻辑保持正常

## 2026-05-07-02

### 这版一句话
发布 Flutter 客户端 `1.0.31`，让 App 用户可以使用“修正余额”和现金扣款调整记录相关更新。

### 主要变化
- Flutter 客户端版本号从 `1.0.30` 升级到 `1.0.31`
- Android `versionCode` 随语义版本推导为 `31`，避免和上一版 `1.0.30` 安装包撞版本

### 影响范围
- App：Android 安装包版本识别和用户升级安装

### 验收重点
- 新打包 APK 文件名应为 `kaka-1.0.31-release.apk`
- 旧版 `1.0.30` 用户安装新版 APK 时应被识别为升级

## 2026-05-07-01

### 这版一句话
补齐买入扣款调整记录，并把资产编辑里的余额改动改为可追踪的“修正余额”。

### 主要变化
- 买入/加仓使用现金账户扣款时，会在对应现金账户写入 `买入 {标的名称} 扣款` 调整记录
- 现金、其他资产和负债的调整接口新增 `correct` 模式，用目标余额生成一条增加或减少流水
- App 资产编辑弹窗增加“修正余额”入口，Web 编辑弹层在修改余额时也会写入调整记录
- 撤销买入或卖出回款操作时，会同步删除对应现金账户调整记录，避免流水残留

### 影响范围
- 后端：资产调整记录、买入扣款、卖出回款撤销
- App：资产编辑弹窗、资产余额乐观更新
- Web：我的资产页资产编辑弹层

### 验收重点
- 买入/加仓扣款后，现金账户调整记录应出现 `买入 xxx 扣款`
- 编辑资产选择“修正余额”后，应按目标余额更新，并保留一条修正流水
- 撤销买入或卖出回款后，现金余额和对应调整记录都应回到操作前状态
