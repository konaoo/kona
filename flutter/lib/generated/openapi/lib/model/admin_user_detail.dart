...
class AdminUserDetail {
  ...
  @override
  String toString() => 'AdminUserDetail[id=$id, username=$username, nickname=$nickname, phone=$phone, userNumber=$userNumber, registerMethod=$registerMethod, isAdmin=$isAdmin, mustChangePassword=$mustChangePassword, status=$status, createdAt=$createdAt, lastLogin=$lastLogin, lastActiveAt=$lastActiveAt, lastLoginIp=$lastLoginIp, lastLoginRegion=$lastLoginRegion, lastActiveIp=$lastActiveIp, lastActiveRegion=$lastActiveRegion, activeSessions=$activeSessions, canManage=$canManage]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AdminUserDetail[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AdminUserDetail[$key]" has a null value in JSON.');
        });
        return true;
      }());
