...
class RefreshSessionRequest {
  ...
  @override
  String toString() => 'RefreshSessionRequest[refreshToken=$refreshToken, deviceId=$deviceId]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RefreshSessionRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RefreshSessionRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<RefreshSessionRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<RefreshSessionRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RefreshSessionRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = RefreshSessionRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
